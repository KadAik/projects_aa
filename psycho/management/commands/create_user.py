...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "p_username", type=str, nargs="?", help="The username for the new user."
        )
        parser.add_argument(
            "p_password", type=str, nargs="?", help="The password for the new user."
        )
        ...
        parser.add_argument(
            "--username", type=str, help="The username for the new user.", default=None
        )
        parser.add_argument(
            "-p",
            "--password",
        ...
        parser.add_argument(
            ...
            "--interactive",
            action="store_true",
            help="Create the user interactively.",
        )
    ...
    def handle(self, *args, **options):
        ...
        missing_positional_args = not options.get("p_username") and not options.get(
            "p_password"
        )
        missing_optional_args = not options.get("username") and not options.get(
            "password"
        )
        ...
        if is_interactive or (missing_positional_args and missing_optional_args):
            ...
            # Set staff status to true so the user can login to the admin site
            user.is_staff = True
            user.save()
            self.stdout.write(
